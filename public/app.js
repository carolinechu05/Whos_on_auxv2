...
if (vol && volV) {
  vol.addEventListener('input', e => {
    ...
    if (isAux()) socket.emit('volume', e.target.value / 100);
  });
}
...
if (rateDock) {
  ...
  buttons.forEach(btn => {
    btn.addEventListener('click', () => {
      if (!rating || !players[myId] || players[myId].rated || isAux()) return;
...
socket.on('countdown', d => {
  ...
  countdown.textContent = `${phase} ends in ${sec}s`;
  
  const iv = setInterval(() => {
    sec--;
    if (countdown) countdown.textContent = `${phase} ends in ${sec}s`;
